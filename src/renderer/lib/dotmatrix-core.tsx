...
import "@/renderer/components/dotmatrix-loader.css";
import { useDotMatrixPhases, usePrefersReducedMotion } from "@/renderer/lib/dotmatrix-hooks";
...
export function spiralOffset(angle: number, radiusNormalizedValue: number, amplitude = 2.8): { x: number; y: number; phase: number } {
  ...
  const dots = Array.from({ length: MATRIX_SIZE * MATRIX_SIZE }).map((_, index) => {
    ...
    return (
      <span
        key={index}
        aria-hidden="true"
        className={cx("dmx-dot", !isActive && "dmx-inactive", isBloomDot && "dmx-bloom-dot", dotClassName, animationState.className)}
        style={dotStyle}
      />
    );
  });
